...
function updateStats(data) {
  ...
  for (const feature in featureMap) {
    if (featureMap[feature] > maxCount) {
      maxCount = featureMap[feature];
...
function buildFullReport(item, result) {
  ...
  if (feature.includes("doctor")) {
    recommendations.push("Add nearby doctor recommendation or referral support.");
  }

  if (feature.includes("first-aid")) {
    recommendations.push("Include first-aid guidance for minor urgent situations.");
  }

  if (concernAreas.length === 0) {
    concernAreas.push("No major behavioral concern flagged from current survey.");
  }

  if (recommendations.length === 0) {
    recommendations.push("Continue routine symptom monitoring and consult a doctor if concerns increase.");
  }
...
function renderResponses() {
  ...
  filtered.sort((a, b) => {
    return getRiskPriority(a.calculatedRisk.risk_level) - getRiskPriority(b.calculatedRisk.risk_level);
  });
...
async function loadResponses() {
  ...
  if (error) {
    statusBox.textContent = "Error loading data: " + error.message;
    return;
  }

  if (!data || data.length === 0) {
    statusBox.textContent = "No responses found.";
    return;
  }
  ...
  statusBox.textContent = `Loaded ${allResponses.length} response(s).`;
  renderResponses();
}
